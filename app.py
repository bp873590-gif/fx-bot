...
if not df.empty and len(df) > 20:
    ...
    # [FIXED] 'Date' vs 'Datetime' KeyError Solution
    ...
    # --- 5. 🔥 USER-FRIENDLY ZOOM ENGINE (FIXED) 🔥 ---
    fig.update_layout(
        ...
        plot_bgcolor='#0c0d14',
        dragmode='zoom', # 👈 Ab box bana kar zoom karna simple aur intuitive ho jayega!
        yaxis=dict(side="right", gridcolor="#1f2231", fixedrange=False), 
        xaxis=dict(gridcolor="#1f2231", fixedrange=False) 
    )
    
    # Zoom/Pan buttons ko upar enable kiya hai taaki mobile par jhanjhat na ho
    st.plotly_chart(fig, use_container_width=True, config={
        'scrollZoom': True,           
        'displayModeBar': True, # 👈 Buttons dikhenge, takki click karke zoom out/reset kar sako!
        'modeBarButtonsToRemove': ['select2d', 'lasso2d', 'toggleHover'],
        'doubleClick': 'reset' # Chart par do baar tap karte hi zoom normal ho jayega!
    })
